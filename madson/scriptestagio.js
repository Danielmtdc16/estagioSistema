...
//Parâmetros para determinar a Pressão de vapo d'água
    ...
    //'viscosidade cinemática da água na sucção
        ...
        Uc = u * 1000;
        Label54 = ("Viscosidade dinâmica:  " + Uc + " x 10-³ N.s/m²");
    ...
    //'massa específica e fator de correção da temperatura FCt na sucção
        ...
        Label55 = ("Massa específica:  " + mespa + " kg/m³");
    ...
    //'Derterminação do fator de atrito e do regime de escoamento na sucção
        ...
        while( Math.abs(deltaf / oldf) >= er){
            Newf = 1 / (-2 * Math.log(Rgsuc / (3.7 * Dsuc) + 2.51 / (TextBox10.value * Math.sqrt(oldf))) * 0.434294482) ^ 2;
            deltaf = Newf - oldf;
            oldf = Newf;
        }
            F = oldf;
    ...
    TextBox13.value = (6.376 * (10 ^ 6) * TextBox11.value * (Qsuc ^ 2) * Lsuc / Dsuc ^ 5).toFixed(2);
    Hfls = Math.abs(TextBox13.value) 
    //era um CDbl
        
    //'Perda de carga nas singularidades da sucção

    TextBox14.value = ((Ke + Kvg + kc + Kvpc) * (TextBox12.Text ^ 2) / 19.62).toFixed(2);
    ...
    TextBox16.value = (10.33 * ((293 - 0.0065 * Alt) / 293) ^ 5.26).toFixed(2);
    ...
    Label29 = "Altura estática de sucção máxima = " + Zsmax + " m acima do nível da água no reservatório com NPSHdisp = " + NPSHdreal + "  m";
    ...
    TextBox7.value = (-TextBox9.value * (Zsreal + ((TextBox12.value ^ 2) / 19.62) + HfTs) / 1000).toFixed(2)

    //Em mca
    
    TextBox8.value = (-(Zsreal + ((TextBox12.value ^ 2) / 19.62) + HfTs)).toFixed(2)
    ...
    Label23.Caption = "NPSH disponível (m)"
    ...
    TextBox8.value = (ZsAfog - HfTs - (TextBox12.value ^ 2 / 19.62)).toFixed(2)
    
    //'Em KPa
        
        TextBox7.value = (Pes * TextBox8.value / 1000).toFixed(2)
        
    Label29 = "Altura estática de sucção mínima de: " + ZsAfog + " m abaixo do nível inferior da água no reservatório.";
    //Label29.ForeColor = &H4000&
    ...
    TextBox7.value = (-TextBox9.value * (Zspre + ((TextBox12.value ^ 2) / 19.62) + HfTs) / 1000).toFixed(2)

    //'Em mca
    
    TextBox8.value = (-(Zspre + ((TextBox12.value ^ 2) / 19.62) + HfTs)).toFixed(2)
...
function CommandButton3_Click(){
    ...
    //'Derterminação do fator de atrito e do regime de escoamento no recalque
        ...
        while (Math.abs(deltafr / oldfr) >= er){
            Newfr = 1 / (-2 * Log(Rgrec / (3.7 * Drec) + 2.51 / (TextBox27.value * Sqr(oldfr))) * 0.434294482) ^ 2
            deltafr = Newfr - oldfr
            oldfr = Newfr
        }
            Fr = oldfr
    ...
    TextBox29.value = (353.6775 * Qrec / (Drec ^ 2)).toFixed(2);
    ...
    TextBox30.value = (6.3735 * TextBox28.value * ((1000 * Qrec) ^ 2) * Lrec / Drec ^ 5).toFixed(2);
    HfLr = Math.abs(TextBox30.value);
    
    //'Perda de carga nas singularidades do realque

    TextBox31.Text = ((Kvr + Kvgr + kcr) * (TextBox29.value ^ 2) / 19.62).toFixed(2);
    ...
    TextBox35.value = ((PSaida - PAdm) + (((Vr ^ 2) - (Vsuc ^ 2)) / 19.62) + d).toFixed(2);
